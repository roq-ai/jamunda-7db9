...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  async function getContentManagers() {
    const data = await prisma.content_manager
      .withAuthorization({
        roqUserId,
        tenantId: user.tenantId,
        roles: user.roles,
      })
      .findMany(convertQueryToPrismaUtil(req.query, 'content_manager'));
  ...
  async function createContentManager() {
    ...
    const data = await prisma.content_manager.create({
      data: body,
    });
    return res.status(200).json(data);
  }
