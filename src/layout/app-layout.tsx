...
const SidebarContent = ({ onClose, ...rest }: SidebarProps) => {
  ...
  const tenantRoles = ['administrator', 'content-manager'];
  const isTenantUser = tenantRoles.some((role) => session?.user?.roles?.includes(role));
  const { data, error, isLoading } = useSWR<AdministratorInterface[]>(
    () => (isTenantUser && session?.user?.tenantId ? `/administrators` : null),
    () => getAdministrators({ tenant_id: session?.user?.tenantId }),
  );
  loading = isLoading;
  ...
  const LinkItems: Array<LinkItemProps> = [
    ...
    {
      name: 'Administrator',
      path: isTenantUser ? `/administrators/view/${data?.[0]?.id}` : '/administrators',
      entity: 'administrator',
      service: AccessServiceEnum.PROJECT,
      icon: FiShieldOff,
    },
    { name: 'Content', path: '/contents', entity: 'content', service: AccessServiceEnum.PROJECT, icon: FiVideo },
    {
    ...
    { name: 'Subscriber', path: '/subscribers', entity: 'subscriber', service: AccessServiceEnum.PROJECT, icon: FiRss },
